...
소셜 제공자가 이메일·연락처를 주지 않으면 예약 화면의 `예약 연락처 입력`을 저장한 뒤 바로 예약을 계속할 수 있습니다.
소셜 프로필 이메일이 기존 일반 회원과 같으면 새 고객을 만들지 않고 Google·Naver·Kakao 계정을 한 고객에게 각각 연결해 예약 이력을 유지합니다.

## 업로드와 외부 연동

업로드 기본 경로는 실행 폴더와 무관한 `${user.home}/.marinboy/uploads`입니다. `UPLOAD_DIRECTORY`를 지정할 때는 절대경로만 허용합니다.

실행 스크립트의 Maven 산출물은 `${user.home}/.marinboy/build/v3-backend`, Next production 산출물은 `${user.home}/.marinboy/build/v3-frontend`, 검증 산출물은 `${user.home}/.marinboy/build/verify` 아래에 생성합니다. OneDrive의 `target`·`.next` 잠금과 한글 경로 문제가 재시작에 영향을 주지 않습니다. IDE나 수동 명령은 기존 프로젝트 폴더를 그대로 사용합니다.
...
관리자 화면의 캘린더 주소는 백엔드 설정으로 생성되므로 별도의 공개 프론트 환경변수가 필요하지 않습니다.
