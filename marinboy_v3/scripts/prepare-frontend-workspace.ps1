...
$allowedBuildRoot = [System.IO.Path]::GetFullPath((Join-Path $userProfilePath '.marinboy\build'))
$allowedPrefix = $allowedBuildRoot.TrimEnd('\') + '\'

# 생성 폴더를 사용자 전용 .marinboy/build 아래로 제한해 잘못된 재귀 삭제를 막습니다.
...
foreach ($generatedName in '.next', 'features', 'pages', 'public', 'reducers', 'sagas', 'store', 'styles') {
...
foreach ($directoryName in 'features', 'pages', 'public', 'reducers', 'sagas', 'store', 'styles') {
