...
export default function NewReview() {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      await api.post('/api/reviews', { reservationId: Number(router.query.reservationId), rating, content });
  ...
  return <AppLayout><section className="auth-section container"><form className="paper-form" onSubmit={onSubmit}>
    ...
    <span>별점</span><div className="star-rating" aria-label={`${rating}점 선택됨`}>
      {[1, 2, 3, 4, 5].map((value) => <button key={value} className={value <= rating ? 'star active' : 'star'} data-value={value} type="button" aria-label={`${value}점`} aria-pressed={value === rating} onClick={() => setRating(value)}>★</button>)}
    </div>
